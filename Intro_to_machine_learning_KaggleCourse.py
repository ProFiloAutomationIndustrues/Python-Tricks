...
# one of the best features of Random Forest models is that they generally work reasonably 
# with default parameters.

# EXERCISE RANDOM FORESTS
# Prima trainiamo comunque il DecisionTreeRegressor con optimal_size, per fare poi il confronto con il 
# modello successivo.
...
​
# Specify Model
iowa_model = DecisionTreeRegressor(random_state=1) # NOT SPECIFYING max_leaf_nodes
# Fit Model
iowa_model.fit(train_X, train_y)
​
# Make validation predictions and calculate mean absolute error
...
iowa_model = DecisionTreeRegressor(max_leaf_nodes = 100, random_state = 1) # SPECIFYING max_leaf_nodes
iowa_model.fit(train_X, train_y)
val_predictions = iowa_model.predict(val_X)
val_mae = mean_absolute_error(val_predictions, val_y)
